//Сортировка пузырьком сначала по должности, потом по стажу работы, потом по зарплате.
...
//сортировка
std::vector<EmployeeData> bubbleSort(std::vector<EmployeeData>& dataToSort) {
    for (int i = 0; i < dataToSort.size() - 1; i++) {
        for (int j = 0; j <dataToSort.size()-i-1;j++) {
            if (dataToSort[j].Position > dataToSort[j + 1].Position ||
               (dataToSort[j].Position == dataToSort[j + 1].Position 
               && dataToSort[j].WorkExperience > dataToSort[j + 1].WorkExperience) ||
               (dataToSort[j].Position == dataToSort[j + 1].Position 
               && dataToSort[j].WorkExperience == dataToSort[j + 1].WorkExperience 
               && dataToSort[j].Salary > dataToSort[j + 1].Salary)) {
...
int main() {
    ...
    std::vector<EmployeeData> sortedEmployees = bubbleSort(employees);
